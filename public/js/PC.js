...
PC.mapManager = (function(mapManager, $, undefined){
	...
	var loadBars = function(pubs){
        ...
        for (id in pubs){
			...
			marker.set('id', pubs[id].id);
			markers[pubs[id].id] = marker;
	...
	mapManager.route = function(config){
		...
		if(config.pubCrawl.length > 1){
			request.origin = config.pubCrawl[0].latlng;
			request.destination = config.pubCrawl[config.pubCrawl.length-1].latlng;
			request.waypoints = [];
			for (var i=1, l = config.pubCrawl.length-1; i<l; i++) {
				request.waypoints.push({
					location: config.pubCrawl[i].latlng,
					stopover: true
				});
			}
